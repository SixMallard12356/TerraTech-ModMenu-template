...
[FriendlyName("Distance/Is Player in Trigger Areas [Multiple]")]
public class uScript_IsPlayerInTriggersMultiple : uScriptLogic
{
	...
	public void In(string[] triggerAreas, ref bool inside)
	{
		bool flag = inside;
		if (m_Encounter.IsNotNull())
		{
			...
			{
				bool flag3 = false;
				foreach (string text in triggerAreas)
				{
					...
					{
						d.LogWarning("uScript_IsPlayerInTriggersMultiple - unable to find trigger called " + text);
					}
				}
				if (!flag3)
				{
