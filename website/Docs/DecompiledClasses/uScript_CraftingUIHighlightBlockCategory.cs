...
public class uScript_CraftingUIHighlightBlockCategory : uScriptLogic
{
	...
	public void AllCategory(BlockCategories unused)
	{
		if (!m_IsSelected && m_FirstFrame)
		{
			InitHelpArrow(BlockCategories.Null);
			m_FirstFrame = false;
		}
	}

	public void Category(BlockCategories blockCategory)
	{
		if (!m_IsSelected && m_FirstFrame)
		{
			InitHelpArrow(blockCategory);
			m_FirstFrame = false;
		}
	}

	private void InitHelpArrow(BlockCategories category)
	{
		...
		{
			m_TargetButton = uIItemRecipeSelect.GetCategoryToggle((int)category);
		}
