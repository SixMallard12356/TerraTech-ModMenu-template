...
public class AStarPathfinding
{
	...
	public AStarPathfinding(IntVector2 maxGridSize)
	{
	...
	public IntVector2[] DoPathfinding(IntVector2 startPos, IntVector2 targetPos, IntVector2 gridStartPos, IntVector2 gridSize, bool useDiagonals, Func<IntVector2, IntVector2, bool> IsWalkable)
	{
		...
		while (true)
		{
			if (m_NoOfOpenListItems != 0)
			{
				...
				for (int i = 0; i < num5; i++)
				{
					...
					if (intVector.x < gridStartPos.x || intVector.x >= gridSize.x || intVector.y < gridStartPos.y || intVector.y >= gridSize.y || !IsWalkable(m_CurrentTilePos, intVector) || m_WhichPathingList[intVector.x, intVector.y] == m_OnClosedListVal)
					{
						continue;
					}
					...
					if (m_WhichPathingList[intVector.x, intVector.y] != m_OnOpenListVal)
					{
						...
						m_HeuristicCost[m_OpenList[m_Index]] = (int)CalculateHeuristicCost(intVector, targetPos);
				...
				if (m_WhichPathingList[targetPos.x, targetPos.y] == m_OnOpenListVal)
				{
					num = kPathFound;
					break;
				}
		...
		if (num == kPathFound)
		{
			IntVector2 intVector2 = targetPos;
			int num6 = 0;
			while (startPos != intVector2)
			{
			...
			num6--;
			array[num6] = m_PathNodePosArray[startPos.x, startPos.y];
		}
		return array;
	}
