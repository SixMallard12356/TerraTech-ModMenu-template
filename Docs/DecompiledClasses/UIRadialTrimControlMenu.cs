...
public class UIRadialTrimControlMenu : UIHUDElement
{
	[SerializeField]
	...
	public override void Show(object context)
	{
		...
		if (IsMenuAvailableForTech(targetTankBlock.tank, targetTankBlock) && openMenuEventData.m_AllowRadialMenu)
		{
			...
			m_Slider.value = m_TargetTank.BlockStateController.GyroTrim * 0.5f + 0.5f;
	...
	public override void Hide(object context)
	{
		...
		m_RadialMenu.Hide();
		m_TargetTank.BlockStateController.GyroTrim = Mathf.Lerp(-1f, 1f, m_Slider.value);
	...
	private void OnOptionSelected(int option)
	{
		...
		{
			bool flag = m_TargetTank.BlockStateController.IsCategoryActive(ModuleControlCategory.GyroTrim);
			m_TargetTank.BlockStateController.RequestSetCategoryActive(ModuleControlCategory.GyroTrim, !flag);
			HideSelf();
		}
	}
	...
	private void OnOptionHovered(int option)
	{
		if (m_RadialController.IsGamePad())
		{
			bool active = option == 0;
			m_Slider.gameObject.SetActive(active);
			if ((bool)m_RightPanel)
			{
				m_RightPanel.SetActive(active);
			}
		}
	}

	private bool IsMenuAvailableForTech(Tank targetTank, TankBlock targetBlock)
	{
		bool num = !Singleton.Manager<ManGameMode>.inst.LockPlayerControls && Singleton.playerTank != null;
		bool flag = targetTank != null && (targetTank == Singleton.playerTank || !targetBlock.ContextMenuForPlayerTechOnly) && targetTank.Team == Singleton.Manager<ManPlayer>.inst.PlayerTeam && targetTank.visible.IsInteractible;
		bool flag2 = false;
	...
	private void Update()
	{
		...
		if (m_RadialMenu.IsModal())
		{
			...
			{
				m_Slider.value += m_SliderGamepadSensitivity * Singleton.Manager<ManInput>.inst.GetAxis(19);
			}
		}
		m_TargetTank.BlockStateController.GyroTrim = Mathf.Lerp(-1f, 1f, m_Slider.value);
	}
